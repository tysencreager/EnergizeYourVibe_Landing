...
- Link previews: `functions/events/[slug].js` serves each event page with
  its own title, description and share image in the `<head>` (social
  crawlers don't run the app's JavaScript, so without it every link would
  preview as the generic site card). Share images are
  `public/assets/og/<slug>.jpg`, generated by `scripts/og-images.mjs`.
...
2. Optional short link: add a line to `public/_redirects` above the SPA
   fallback.
   Share image: run `node scripts/og-images.mjs <slug>` (needs Playwright:
   `npm i -D playwright && npx playwright install chromium`, or ask Claude
   to run it) and set `ogImage` on the event to the generated
   `public/assets/og/<slug>.jpg`. Commit the JPG. To restyle the cards,
   edit the template in the script and re-run it with no arguments.
